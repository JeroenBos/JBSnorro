...
namespace JBSnorro.Commands
{
	...
	public static class ViewModelCommandExtensions
	{
		...
		{
			string formattedCommandName = commandName == null ? null : $"'{commandName}' ";
		...
		internal static string getCommandName(this object command)
		{
			...
			if (command is INamedCommand namedCommand)
			{
				string name = namedCommand.CommandName;
			...
			if (result != command.GetType().ToString())
				return result;
			if (result.Contains("`"))
				result = result.Substring(0, result.IndexOf('`')); // remove generic type parameter
			if (result.Contains("."))
				result = result.Substring(result.LastIndexOf('.') + 1); // remove qualifiers
			if (result.EndsWith("Command"))
				result = result.Substring(0, result.Length - "Command".Length); // remove postfix "Command"
			return result;
		}
