...
namespace JBSnorro.Diagnostics
{
    ...
    public static class Contract
    {
        ...
        private static TException Create<TException>(string message)
        {
            ...
            Contract.Assert(ctor != null); // prevents warning
        ...
        public static void LazilyRequireAny<T>(ref IEnumerable<T> sequence, Func<T, bool> predicate, string? message = null)
        {
            ...
            else
            {
                sequence = sequence.Select(predicateThatStoresResult)
                                   .ContinueWith(() => { if (message == null) Assert(anyElementMatchedPredicate); else Assert(anyElementMatchedPredicate, message); });
            }
        }
        ...
        public static void AssertCaller(string callerName)
        {
            ...
            Assert(caller != null); // prevents warning
            Assert(caller.DeclaringType != null); // prevents warning

            // actual assertion that fulfills the contract of this method:
