...
public static class EqualityExtensions
{
    ...
    /// <summary>
    /// Compares the specified function for equality with the provided expected samples, up to a specified tolerance.
    /// </summary>
    [DebuggerHidden]
    public static bool SamplewiseEquals<TInput>(this Func<TInput, float> function, Dictionary<TInput, float> sampledExpectations, float tolerance = tolerance) where TInput : notnull
    {
        return sampledExpectations.All([DebuggerHidden] (s) => function(s.Key).ApproximatelyEquals(s.Value, tolerance));
        // foreach (var (sample, expectation) in sampledExpectations)
        // {
        //     var value = function(sample);
        //     if (!value.ApproximatelyEquals(expectation, tolerance))
        //         return false;
        // }
        // return true;
    }
